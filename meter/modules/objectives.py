...
def compute_snli(pl_module, batch):
    ...
    return ret
def compute_vqa(pl_module, batch, test=True):
    if pl_module.hparams.config["kpl_meter"]:
        infer = pl_module.infer_prompt(batch, mask_text=False, mask_image=False)
    else:
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
...
def computer_irtr_knowledge(pl_module, batch, test=True):
    ...
    # prompt-false
    prompt_text_ids = torch.stack(
        [batch[f"false_prompt_text_{i}_ids"] for i in range(false_len)], dim=1
    )
    prompt_masks = torch.stack(
        [batch[f"false_prompt_text_{i}_masks"] for i in range(false_len)], dim=1
    )
    ...
    # prompt
    prompt_masks = torch.cat([batch["prompt_text_masks"].unsqueeze(1), prompt_masks], dim=1)
    prompt_text_ids = torch.cat([batch["prompt_text_ids"].unsqueeze(1), prompt_text_ids], dim=1)

    images = batch["image"][0].unsqueeze(1).expand(_bs, false_len + 1, _c, _h, _w)
    infer = pl_module.infer_prompt(
        {
            ...
            "prompt_str_ids": rearrange(prompt_text_ids, "bs fs tl -> (bs fs) tl"),
            "prompt_str_masks": rearrange(prompt_masks, "bs fs tl -> (bs fs) tl"),
        }
    )
...
def computer_irtr_no_knowledge(pl_module, batch, test=True):
...
@torch.no_grad()
def compute_irtr_recall_no_knowledge(pl_module):
    text_dset = pl_module.trainer.datamodule.dms[0].make_no_false_val_dset()
    text_dset.tokenizer = pl_module.trainer.datamodule.dms[0].tokenizer
    text_loader = torch.utils.data.DataLoader(
        text_dset,
        batch_size=pl_module.hparams.config["irtr_recall_batch_size"],
...
@torch.no_grad()
def compute_irtr_recall_knowledge(pl_module):
    text_dset = pl_module.trainer.datamodule.dms[0].make_no_false_val_dset()
    text_dset.tokenizer = pl_module.trainer.datamodule.dms[0].tokenizer
    text_loader = torch.utils.data.DataLoader(
        text_dset,
        ...
        text_preload.append(
            {
                ...
                "text_labels": _b["text_labels"].to(pl_module.device),
                "img_index": _b["img_index"],
                "prompt_str_ids": _b["prompt_text_ids"].to(pl_module.device),
                "prompt_str_masks": _b["prompt_text_masks"].to(pl_module.device),
            }
        )
    ...
    for img_batch in tqdm.tqdm(image_preload, desc="rank loop"):
        ...
        for txt_batch in text_preload:
            ...
            with torch.cuda.amp.autocast():
                score = pl_module.rank_output(
                    pl_module.infer_prompt(
                        {
                            "text_ids": txt_batch["text_ids"],
                            "text_masks": txt_batch["text_masks"],
                            "text_labels": txt_batch["text_labels"],
                            "prompt_str_ids": txt_batch["prompt_str_ids"],
                            "prompt_str_masks": txt_batch["prompt_str_masks"],
                        },
                        img=im,
                    )["cls_feats"]
                )[:, 0]
