...
from .clip_model_prompt import VisualTransformer_prompt
from .clip_model_adapter import VisualTransformer_adapter
from .clip_model_adaptformer import VisualTransformer_adaptFormer
from .clip_model_vpt import VisualTransformer_vpt
from .clip_model_lora import VisualTransformer_lora
from .clip_model_lora_layers import PlainMultiheadAttentionLoRA
from .clip_model_uniadapter import VisualTransformer_uniadapter
    ...
    def __init__(self,
        ...
        if config["kpl_meter"]:
            self.visual = VisualTransformer_adapter(
                ...
                adapter_factor=config["adapter_factor"],
            )
        else:
...
def build_model(name, resolution_after=224, config=None, uniadapters=None):
    if name in _MODELS:
    ...
    model = CLIP(
        ...
        resolution_after, config=config, uniadapters=uniadapters,
    )
