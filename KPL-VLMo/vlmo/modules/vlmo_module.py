...
class VLMo(pl.LightningModule):
    def __init__(self, config):
        ...
        if self.hparams.config["use_adapter"]:
            for param in self.parameters():
                param.requires_grad = False
            # need_grads = ["vqa_classifier", "pooler"]
            need_grads = ["image_adapter", "text_adapter", "multi_adaper", "vqa_classifier"]
    ...
    def infer(
        ...
        if self.hparams.config["use_adapter"]:
            ...
            prompt_text_embeds = self.text_embeddings(prompt_text_ids)
        ...
        if self.hparams.config["use_adapter"]:
            ...
            # prompt_text_embeds = torch.cat([prompt_text_embeds, image_embeds], dim=1)
            prompt_masks = torch.cat([batch["prompt_str_masks"], image_masks], dim=1)
        ...
        for i, blk in enumerate(self.transformer.blocks):
            x, prompt_text_embeds = blk(x, prompt_embeds=prompt_text_embeds, prompt_masks=prompt_masks, mask=co_masks, modality_type="vl", relative_position_bias=relative_position_bias_list[i])
    ...
    def infer_text_ft(
        ...
        # if self.hparams.config["use_adapter"]:
        #     prompt_text_embeds = prompt_text_embeds + self.token_type_embeddings(torch.zeros_like(text_masks))
        #     prompt_masks = batch["prompt_str_masks"]

        for i, blk in enumerate(self.transformer.blocks):
            x, prompt_text_embeds  = blk(x, prompt_embeds=prompt_text_embeds, prompt_masks=prompt_masks,mask=co_masks, modality_type="text", relative_position_bias=relative_position_bias_list[i])
            all_hidden_states.append(x)
    ...
    def infer_image_ft(
        ...
        relative_position_bias_list = self.get_rel_pos_bias(self.relative_position_index)
        for i, blk in enumerate(self.transformer.blocks):
            x, _ = blk(x, mask=co_masks, modality_type="image", relative_position_bias=relative_position_bias_list[i])
            all_hidden_states.append(x)
    ...
    def test_epoch_end(self, outs):
        ...
        vlmo_utils.epoch_wrapup(self, test=True)
