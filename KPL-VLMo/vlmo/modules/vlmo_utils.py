...
from vlmo.gadgets.my_metrics import Accuracy, VQARADScore, Scalar
        ...
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                if split == "train":
                    ...
                    setattr(pl_module, f"train_{k}_i2t_accuracy", Accuracy())
                    setattr(pl_module, f"train_{k}_t2i_accuracy", Accuracy())
                else:
                    setattr(pl_module, f"val_irtr_loss", Scalar())
                    setattr(pl_module, f"test_irtr_loss", Scalar())
                    setattr(pl_module, f"val_{k}_i2t_accuracy", Accuracy())
                    setattr(pl_module, f"val_{k}_t2i_accuracy", Accuracy())
                    setattr(pl_module, f"test_{k}_i2t_accuracy", Accuracy())
                    setattr(pl_module, f"test_{k}_t2i_accuracy", Accuracy())
            elif k == "itm":
...
def set_schedule(pl_module):
    ...
    adapter_related_names = ["image_adapter", "text_adapter", "multi_adaper"]
